...
import { operatorsHorizontal, operatorsVertical, numKeypads, otherSymbols } from './constant.json';

const Index = ({hasSwitchedView}) => {
  ...
  const [hasNegativeOperatorSelected, setNegativeOperatorSelected] = useState(false);
  ...
  const handleOperator = (e, operator)=> {
    ...
    if(operator.name === 'add' || operator.name === 'subtract' || operator.name === 'divide' || operator.name === 'multiplication'){
      setSelectedNumber(0);
      setDecimalSelected(false);
      setNegativeOperatorSelected(false);
    ...
    if(operator.name === 'equals'){
      ...
      const replacedExpression = expressions.replace(/parseFloat/,'').replace(/[(]/, '').replace(/[)]/, '');
      const addHistoryToList = [...history, { expressions: replacedExpression, result: result}]
  ...
  const handleNumPad = (e, keypad) => {
    ...
    if(displayNumber === 0 && (keypad.value === null || keypad.value === 0)) return ;
    ...
    //if operator has selected, then add operater and keyCotent to expression
    if(hasOperatorSelected) {
      setExpressions(expressions + selectedOperator.operator + keyContent);
    } else {
      //check if operter exist in expression
    ...
    <Calculator landscapeView={hasSwitchedView}>
      ...
      <CalculatorKeyPad>
        <BigColumn landscapeView={hasSwitchedView}>
          {otherSymbols.map(otherSymbol => <Button otherKeys key={otherSymbol.name}>{otherSymbol.symbol}</Button>)}
        </BigColumn>
        <FirstColumn>
          <Block>
            {operatorsHorizontal.map(operator =>
              <Button horizontalOperator key={operator.name} onClick={(e) => handleOperator(e, operator)}>{operator.symbol}</Button>
            )}
          </Block>
          <Block>
            { numKeypads.map(keyPad => <__NumPadButtons numPads key={keyPad.name} onClick={(e) => handleNumPad(e, keyPad)}>{keyPad.name}</__NumPadButtons>)}
          </Block>
        </FirstColumn>
        <SecondColumn>
          <VerticalOperatorWrapper>
            {operatorsVertical.map(operator =>
              <Button verticalOperator key={operator.name} onClick={(e) => handleOperator(e, operator)}>{operator.symbol}</Button>
            )}
          </VerticalOperatorWrapper>
        </SecondColumn>
      </CalculatorKeyPad>
